...
class Policy(BasePolicy):
    def __init__(
        ...
        *,
        ...
        self._rtc_conditioner = RTCSessionConditioner(
            enabled=rtc_chunk_conditioning,
            fixed_delay_steps=rtc_delay_steps,
            soft_horizon_steps=rtc_soft_horizon_steps,
            free_tail_steps=rtc_free_tail_steps,
        )
        self._metadata = {
            **self._metadata,
            "rtc": self._rtc_conditioner.metadata,
        }
    ...
    def _prepare_rtc_condition(self, rtc_meta: dict[str, Any] | None) -> tuple[dict[str, Any], dict[str, Any] | None]:
        ...
        result, response = self._rtc_conditioner.prepare(
            rtc_meta,
    ...
    def _finalize_rtc_outputs(self, raw_actions: np.ndarray, rtc_response: dict[str, Any] | None) -> tuple[np.ndarray, dict[str, Any] | None]:
        return self._rtc_conditioner.finalize(raw_actions, rtc_response)
