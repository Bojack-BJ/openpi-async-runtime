...
def main() -> None:
    ...
    parser.add_argument("--plan_servo_max_joint_velocity_rad_s", type=float, default=15.0, help="计划 joint 最大速度校验阈值；默认 1.0rad/s，0 表示关闭校验")
    parser.add_argument("--plan_servo_max_joint_acceleration_rad_s2", type=float, default=100.0, help="计划 joint 最大加速度校验阈值；默认 40rad/s^2，0 表示关闭校验")
    ...
    def inference_loop() -> None:
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                if not args.no_gripper:
                    actions_all = base._adjust_gripper_actions(
                        actions_all,
                        arm_mode=args.arm_mode,
                        single_arm_index=single_arm_index,
                    )
    ...
    def control_loop() -> None:
        ...
        while not stop_event.is_set():
            ...
            limited_action, limit_info = limit_action_step(
                ...
                max_gripper_step=0.0 if args.no_gripper else args.max_gripper_step,
            )
            readback_enabled = (
