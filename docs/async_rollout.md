...
This is the full OpenPI hardware-integration guide. Client command paths point to the reference snapshots under `examples/openpi`. The policy server command still refers to `scripts/serve_policy.py` in an OpenPI checkout; it is not part of this runtime package.

The examples directly control robot hardware. Review vendor SDK paths, IP addresses, action units, safety limits, and emergency-stop behavior before running them.
...
`examples/openpi/xarm/pi0_rollout_client_xarm_rpy.py` 的 normal 版默认用 `set_position(..., wait=False)`。`wait=False` 只是不阻塞 Python，并不保证 xArm 控制器立刻丢弃旧目标；如果 for-loop 很快塞入一整个 action chunk，机械臂可能还在执行旧 chunk，Python 已经开始下一轮推理，于是 chunk 切换时会看起来“回退”。

临时缓解方式是启用客户端插值和 `dt` 限速，让每个目标按固定节奏下发：

```bash
python examples/openpi/xarm/pi0_rollout_client_xarm_rpy.py \
...
```bash
python examples/openpi/fasttouch/pi0_rollout_client_fasttouch_rpy.py \
  --description "<task>" \
...
`examples/openpi/xarm/pi0_rollout_client_xarm_rpy_async.py` 额外支持：
...
python examples/openpi/xarm/pi0_rollout_client_xarm_rpy_async.py \
  ...
  --plan_servo_chunk_max_waypoints 50 \
...
- `--plan_servo_chunk_max_waypoints`：每次重规划读取的最大连续未来 action 数，默认 `50`。在 `20Hz` 下覆盖 `2.5s`，避免推理延迟超过短轨迹窗时频繁进入 stale fallback。
...
- `--plan_servo_urdf`：Pinocchio backend 使用的静态 URDF。默认使用仓库内的 `examples/openpi/xarm/assets/xarm6_kinematics.urdf`，它从官方 `xarm_ros2/xarm_description` 的 xArm6 默认参数展开，只保留运动学链。
...
- `--plan_servo_pinocchio_tolerance`：Pinocchio IK 的 SE(3) residual 收敛阈值，默认 `1e-4`。
